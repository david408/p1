def b64e(byts: bytearray) -> bytearray:
    """
    :return: byts converted to a bytearray of b64 values
    """
    l = 4 * int(len(byts) / 3)
    if len(byts) % 3 != 0:
        l += (len(byts) % 3) + 1
...
def unprint(s: str, filesafe: bool = False) -> bytearray:
    """
...
def test_b64():
    ...
    assert sprint(out) == 'SGVsbG8gV29ybGQhIQ'
    assert out == unprint(sprint(out))
    assert b64d(out) == b'Hello World!!'
    s1c1 = "49276d206b696c6c696e6720796f757220627261696e206c696b65206120706f69736f6e6f7573206d757368726f6f6d"
    assert sprint(b64e(bytearray.fromhex(s1c1))) == 'SSdtIGtpbGxpbmcgeW91ciBicmFpbiBsaWtlIGEgcG9pc29ub3VzIG11c2hyb29t'
